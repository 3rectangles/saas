...
import com.barraiser.onboarding.scheduling.scheduling.match_interviewers.InterviewerSorter_V4;
...
public class InterviewerSorter_V4Test {
	@InjectMocks
	private InterviewerSorter_V4 interviewersSorter_v4;
	@Spy
	...
	public void shouldGiveHigherPriorityIfExpertIsDemoEligibleAndInterviewIsDemo() throws IOException {
		...
		final List<InterviewerData> sortedList = this.interviewersSorter_v4.sort(testData.getInterviewers(), true,
				null);
	...
	public void shouldGiveHigherPriorityToExpertHavingHigherProficiencyIfNotDemoInterview() throws IOException {
		final InterviewerSorterTestData testData = this.testingUtil.getTestingData(
				...
				InterviewerSorterTestData.class);
		final List<InterviewerData> sortedList = this.interviewersSorter_v4.sort(testData.getInterviewers(), false,
				null);
		final List<InterviewerData> expectedList = testData.getExpertListOfPriorityInterviewers();
		assertEquals(
				expectedList.stream().map(InterviewerData::getId).collect(Collectors.toList()),
	...
	@Test
	public void shouldGivePriorityToExpertHavingLowerCostIfFallbackEnabled() throws IOException {
		final InterviewerSorterTestData testData = this.testingUtil.getTestingData(
				"src/test/resources/json_data_files/InterviewerSortBasedOnCostAndFallbackTestData.json",
				InterviewerSorterTestData.class);
		final List<InterviewerData> sortedList = this.interviewersSorter_v4.sort(testData.getInterviewers(), false,
				Boolean.TRUE);
		final List<InterviewerData> expectedList = testData.getExpertListOfPriorityInterviewers();
