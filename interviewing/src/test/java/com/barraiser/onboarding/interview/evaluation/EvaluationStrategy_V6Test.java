...
import com.barraiser.onboarding.interview.evaluation.scores.EvaluationStrategy_V6;
...
public class EvaluationStrategy_V6Test {

	@InjectMocks
	private EvaluationStrategy_V6 evaluationStrategy_v6;
	...
	public void testEvaluationStrategy() throws Exception {
		...
		final List<SkillScore> evaluationScoreList = this.evaluationStrategy_v6.computeEvaluationScore(input)
				.getSkillScores();

		for (final SkillScore evaluationScore : evaluationScoreList) {
			if ("1".equalsIgnoreCase(evaluationScore.getSkillId())) {
				assertEquals(574D, evaluationScore.getScore().doubleValue(), 0.0);
			}
			if ("2".equalsIgnoreCase(evaluationScore.getSkillId())) {
				assertEquals(730D, evaluationScore.getScore().doubleValue(), 0.0);
			}
			if ("3".equalsIgnoreCase(evaluationScore.getSkillId())) {
				assertEquals(330D, evaluationScore.getScore().doubleValue(), 0.0);
			}
			if ("52".equalsIgnoreCase(evaluationScore.getSkillId())) {
				assertEquals(450D, evaluationScore.getScore().doubleValue(), 0.0);
			}
		}
	}
}
