...
import com.barraiser.onboarding.interview.evaluation.scores.EvaluationStrategy_V9;
...
public class EvaluationStrategy_V9Test {
	@InjectMocks
	private EvaluationStrategy_V9 evaluationStrategy_v9;
	...
	public void testEvaluationStrategy() throws Exception {
		...
		final ComputeEvaluationScoresData input = ComputeEvaluationScoresData.builder()
				...
				.softSkillFeedbackList(
						Arrays.asList(FeedbackData.builder()
								.id(UUID.randomUUID().toString())
								.categoryId("52")
								.rating(8.0F)
								.normalisedRating(4.4F)
								.difficulty("MODERATE")
								.feedback(UUID.randomUUID().toString())
								.build()))
				.build();

		final List<SkillScore> evaluationScoreList = this.evaluationStrategy_v9.computeEvaluationScore(input)
				.getSkillScores();
		...
		for (final SkillScore evaluationScore : evaluationScoreList) {
			...
			if ("52".equalsIgnoreCase(evaluationScore.getSkillId())) {
				assertEquals(414D, evaluationScore.getScore().doubleValue(), 0.0);
			}
		}
	}
}
