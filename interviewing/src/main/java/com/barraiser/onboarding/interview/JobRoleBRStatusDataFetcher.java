...
@Component
@Log4j2
public class JobRoleBRStatusDataFetcher extends AuthorizedGraphQLQuery<List<StatusType>> {
	private final GraphQLUtil graphQLUtil;
	private final AuthorizationServiceFeignClient authorizationServiceFeignClient;
	private final StatusRepository statusRepository;
	private final StatusMapper statusMapper;

	public JobRoleBRStatusDataFetcher(AuthorizationServiceFeignClient authorizationServiceFeignClient,
	...
	protected List<StatusType> fetch(DataFetchingEnvironment environment, AuthorizationResult authorizationResult) {
		...
		return statusDAOS.stream().filter(s -> this.statusMapper.matchContext(s, "pure_saas", "brStatus"))
	...
	public List<List<String>> typeNameMap() {
		return List.of(
				List.of(QUERY_TYPE, "fetchJobRoleBRStatus"));
	}
}
