...
import com.barraiser.onboarding.interview.evaluation.scores.EvaluationStrategy_V9Constants;
...
@Component
@AllArgsConstructor
public class EvaluationStrategy_V13 implements EvaluationStrategy {
	public static final String SCORING_ALGO_VERSION = "13";

	private final EvaluationScoreComputationManager evaluationScoreComputationManager;
	private final FeedbackLengthFlagCalculator_V13 feedbackLengthFlagCalculator_v13;
	...
	private Map<String, Double> getAggregateScorePerCategory(final List<InterviewData> interviews,
		...
		softSkillsFeedback.setLengthFlag(EvaluationStrategy_V13Constants.FeedbackLengthFlag.MID.getValue());
		this.updateFeedbackScoreAndWeightsPerCategory(
				softSkillsFeedback,
				EvaluationStrategy_V9Constants.QuestionType.REQUIRED.getValue(),
				sumOfFeedbackScorePerCategory,
				sumOfFeedbackWeightsPerCategory, EvaluationStrategy_V13Constants.QuestionTimeSpentFlag.MID.getValue());
	...
	private Float getFeedbackScore(final FeedbackData feedback) {
		final Float normalisedRating = feedback.getIsSaasFeedback() ? feedback.getRating()
				: this.feedbackNormalisationUtil.getCappedNormalisedRating(feedback,
						NormalisationVersionFetcher.getNormalisationAlgoVersion(SCORING_ALGO_VERSION));
		final EvaluationStrategy_V13Constants.FeedbackDifficulty difficulty = EvaluationStrategy_V13Constants.FeedbackDifficulty
				.fromString(
						feedback.getDifficulty());
		final int scoreFactor;
		if (feedback.getIsSaasFeedback()) {
			scoreFactor = normalisedRating > 5 ? 70 : 100;
		} else {
			switch (difficulty) {
				case EASY:
	...
	private Double getFeedbackWeightage(final FeedbackData feedback, final String type,
		...
		final Float normalisedRating = feedback.getIsSaasFeedback() ? feedback.getRating()
				: this.feedbackNormalisationUtil.getCappedNormalisedRating(feedback,
						NormalisationVersionFetcher.getNormalisationAlgoVersion(SCORING_ALGO_VERSION));
		final Double modifiedWeightageOfQuestion = QuestionTypeWeightageCalculator_V13.getModifiedWeightage(
				EvaluationStrategy_V13Constants.QuestionType.fromString(type), normalisedRating);
		final Double modifiedWeightageOfFeedback = FeedbackWeightageCalculator_V13.getModifiedWeightage(
				feedback,
				normalisedRating);
		final Double modifiedWeightageOfQuestionTimeAndFeedbackLength = ModifiedWeightageCalculator_V13
				.getModifiedWeightageForFeedbackLengthAndQuestionTime(
						feedback,
						EvaluationStrategy_V13Constants.QuestionTimeSpentFlag.fromString(timeSpentInQuestionFlag));
	...
	private boolean isQuestionToBeConsidered(final QuestionData question) {
		return !(EvaluationStrategy_V13Constants.QuestionType.DELETED
				.getValue()
				.equals(question.getType())
				|| EvaluationStrategy_V13Constants.QuestionType.NON_EVALUATIVE
						.getValue()
						.equals(question.getType()));
	}
	...
	private List<InterviewData> populateFlagsForScoreCalculation(final List<InterviewData> interviews) {
		for (int j = 0; j < interviews.size(); j++) {
			...
			final List<QuestionData> questions = this.feedbackLengthFlagCalculator_v13.calculate(
	...
	private List<QuestionData> populateTimeSpentFlags(final List<QuestionData> questions,
		...
		for (int i = 0; i < nonDeletedQuestions.size() - 1; i++) {
			final String timeSpentFlag = QuestionTimeSpentFlagCalculator_V13
		...
		nonDeletedQuestions.set(nonDeletedQuestions.size() - 1,
				nonDeletedQuestions.get(nonDeletedQuestions.size() - 1).toBuilder()
						.timeSpentFlag(QuestionTimeSpentFlagCalculator_V13
	...
	private List<QuestionData> filterDeletedQuestions(final List<QuestionData> questions) {
		return questions.stream().filter(x -> !EvaluationStrategy_V13Constants.QuestionType.DELETED.getValue()
