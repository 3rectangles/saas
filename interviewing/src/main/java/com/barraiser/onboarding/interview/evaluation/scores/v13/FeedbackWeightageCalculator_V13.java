...
public class FeedbackWeightageCalculator_V13 {
	...
	private static final List<Map<String, Object>> difficultyToWeightageMapping = List.of(
			Map.of(
					FEEDBACK_DIFFICULTY, EvaluationStrategy_V13Constants.FeedbackDifficulty.HARD,
			...
			Map.of(
					...
					MODIFIED_WEIGHTAGE, 1.5D),
			Map.of(
					FEEDBACK_DIFFICULTY, EvaluationStrategy_V13Constants.FeedbackDifficulty.MODERATE,
			...
			Map.of(
					...
					MODIFIED_WEIGHTAGE, 2D),
			Map.of(
					FEEDBACK_DIFFICULTY, EvaluationStrategy_V13Constants.FeedbackDifficulty.EASY,
					RATING_LOWER_BOUND, 9D,
					RATING_UPPER_BOUND, 10D,
					MODIFIED_WEIGHTAGE, 1.5D),
			Map.of(
	...
	public static Double getModifiedWeightage(final FeedbackData feedback, final Float rating) {
		...
		final EvaluationStrategy_V13Constants.FeedbackDifficulty difficulty = EvaluationStrategy_V13Constants.FeedbackDifficulty
				.fromString(feedback.getDifficulty());
		final double roundedRating = Math.floor(rating * 1000) / 1000D;
		for (Map<String, Object> obj : difficultyToWeightageMapping) {
			if (obj.get(FEEDBACK_DIFFICULTY).equals(difficulty) && roundedRating > (Double) obj.get(RATING_LOWER_BOUND)
					&&
