...
@Component
@AllArgsConstructor
public class EvaluationStrategy_V11 implements EvaluationStrategy {
	public static final String SCORING_ALGO_VERSION = "11";

	private final EvaluationScoreComputationManager evaluationScoreComputationManager;
	private final FeedbackLengthFlagCalculator_V11 feedbackLengthFlagCalculator_v11;
	...
	private Map<String, Double> getAggregateScorePerCategory(final List<InterviewData> interviews,
		...
		softSkillsFeedback.setLengthFlag(EvaluationStrategy_V11Constants.FeedbackLengthFlag.MID.getValue());
		this.updateFeedbackScoreAndWeightsPerCategory(
				softSkillsFeedback,
				EvaluationStrategy_V9Constants.QuestionType.REQUIRED.getValue(),
				sumOfFeedbackScorePerCategory,
				sumOfFeedbackWeightsPerCategory, EvaluationStrategy_V11Constants.QuestionTimeSpentFlag.MID.getValue());
	...
	private Float getFeedbackScore(final FeedbackData feedback) {
		...
		final EvaluationStrategy_V11Constants.FeedbackDifficulty difficulty = EvaluationStrategy_V11Constants.FeedbackDifficulty
				.fromString(
	...
	private Double getFeedbackWeightage(final FeedbackData feedback, final String type,
		...
		final Float normalisedRating = this.feedbackNormalisationUtil.getCappedNormalisedRating(feedback,
				NormalisationVersionFetcher.getNormalisationAlgoVersion(SCORING_ALGO_VERSION));
		final Double modifiedWeightageOfQuestion = QuestionTypeWeightageCalculator_V11.getModifiedWeightage(
				EvaluationStrategy_V11Constants.QuestionType.fromString(type), normalisedRating);
		final Double modifiedWeightageOfFeedback = FeedbackWeightageCalculator_V11.getModifiedWeightage(
				EvaluationStrategy_V11Constants.FeedbackDifficulty.fromString(feedback.getDifficulty()),
				normalisedRating);
		final Double modifiedWeightageOfQuestionTimeAndFeedbackLength = ModifiedWeightageCalculator_V11
				.getModifiedWeightageForFeedbackLengthAndQuestionTime(
						EvaluationStrategy_V11Constants.FeedbackLengthFlag.fromString(feedback.getLengthFlag()),
						EvaluationStrategy_V11Constants.QuestionTimeSpentFlag.fromString(timeSpentInQuestionFlag));
	...
	private boolean isQuestionToBeConsidered(final QuestionData question) {
		return !(EvaluationStrategy_V11Constants.QuestionType.DELETED
				.getValue()
				.equals(question.getType())
				|| EvaluationStrategy_V11Constants.QuestionType.NON_EVALUATIVE
						.getValue()
						.equals(question.getType()));
	}
	...
	private List<InterviewData> populateFlagsForScoreCalculation(final List<InterviewData> interviews) {
		for (int j = 0; j < interviews.size(); j++) {
			...
			final List<QuestionData> questions = this.feedbackLengthFlagCalculator_v11.calculate(
	...
	private List<QuestionData> populateTimeSpentFlags(final List<QuestionData> questions,
		...
		for (int i = 0; i < nonDeletedQuestions.size() - 1; i++) {
			final String timeSpentFlag = QuestionTimeSpentFlagCalculator_V11
		...
		nonDeletedQuestions.set(nonDeletedQuestions.size() - 1,
				nonDeletedQuestions.get(nonDeletedQuestions.size() - 1).toBuilder()
						.timeSpentFlag(QuestionTimeSpentFlagCalculator_V11
	...
	private List<QuestionData> filterDeletedQuestions(final List<QuestionData> questions) {
		return questions.stream().filter(x -> !EvaluationStrategy_V11Constants.QuestionType.DELETED.getValue()
