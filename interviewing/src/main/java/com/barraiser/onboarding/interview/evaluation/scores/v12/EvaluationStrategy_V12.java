...
@Component
@AllArgsConstructor
public class EvaluationStrategy_V12 implements EvaluationStrategy {
	public static final String SCORING_ALGO_VERSION = "12";

	private final EvaluationScoreComputationManager evaluationScoreComputationManager;
	private final FeedbackLengthFlagCalculator_V12 feedbackLengthFlagCalculator_v12;
	...
	private Map<String, Double> getAggregateScorePerCategory(final List<InterviewData> interviews,
		...
		softSkillsFeedback.setLengthFlag(EvaluationStrategy_V12Constants.FeedbackLengthFlag.MID.getValue());
		this.updateFeedbackScoreAndWeightsPerCategory(
				softSkillsFeedback,
				EvaluationStrategy_V9Constants.QuestionType.REQUIRED.getValue(),
				sumOfFeedbackScorePerCategory,
				sumOfFeedbackWeightsPerCategory, EvaluationStrategy_V12Constants.QuestionTimeSpentFlag.MID.getValue());
	...
	private Float getFeedbackScore(final FeedbackData feedback) {
		...
		final EvaluationStrategy_V12Constants.FeedbackDifficulty difficulty = EvaluationStrategy_V12Constants.FeedbackDifficulty
				.fromString(
	...
	private Double getFeedbackWeightage(final FeedbackData feedback, final String type,
		...
		final Float normalisedRating = feedback.getIsSaasFeedback() ? feedback.getRating()
				: this.feedbackNormalisationUtil.getCappedNormalisedRating(feedback,
						NormalisationVersionFetcher.getNormalisationAlgoVersion(SCORING_ALGO_VERSION));
		final Double modifiedWeightageOfQuestion = QuestionTypeWeightageCalculator_V12.getModifiedWeightage(
				EvaluationStrategy_V12Constants.QuestionType.fromString(type), normalisedRating);
		final Double modifiedWeightageOfFeedback = FeedbackWeightageCalculator_V12.getModifiedWeightage(
				feedback,
				normalisedRating);
		final Double modifiedWeightageOfQuestionTimeAndFeedbackLength = ModifiedWeightageCalculator_V12
				.getModifiedWeightageForFeedbackLengthAndQuestionTime(
						feedback,
						EvaluationStrategy_V12Constants.QuestionTimeSpentFlag.fromString(timeSpentInQuestionFlag));
	...
	private boolean isQuestionToBeConsidered(final QuestionData question) {
		return !(EvaluationStrategy_V12Constants.QuestionType.DELETED
				.getValue()
				.equals(question.getType())
				|| EvaluationStrategy_V12Constants.QuestionType.NON_EVALUATIVE
						.getValue()
						.equals(question.getType()));
	}
	...
	private List<InterviewData> populateFlagsForScoreCalculation(final List<InterviewData> interviews) {
		for (int j = 0; j < interviews.size(); j++) {
			...
			final List<QuestionData> questions = this.feedbackLengthFlagCalculator_v12.calculate(
	...
	private List<QuestionData> populateTimeSpentFlags(final List<QuestionData> questions,
		...
		for (int i = 0; i < nonDeletedQuestions.size() - 1; i++) {
			final String timeSpentFlag = QuestionTimeSpentFlagCalculator_V12
		...
		nonDeletedQuestions.set(nonDeletedQuestions.size() - 1,
				nonDeletedQuestions.get(nonDeletedQuestions.size() - 1).toBuilder()
						.timeSpentFlag(QuestionTimeSpentFlagCalculator_V12
	...
	private List<QuestionData> filterDeletedQuestions(final List<QuestionData> questions) {
		return questions.stream().filter(x -> !EvaluationStrategy_V12Constants.QuestionType.DELETED.getValue()
