...
public class WhatsappRecipientForExpertFromInterviewFetcher implements RecipientFetcher<WhatsappRecipient> {
	...
	public WhatsappRecipient getRecipient(final Entity entity, final String eventType) {
		...
		final Interviewer interviewer = interview.getInterviewer();

		return WhatsappRecipient.builder()
				.toPhoneNumber(interviewer.getUserDetails().getWhatsappNumber() != null
						? interviewer.getUserDetails().getWhatsappNumber()
						: interviewer.getUserDetails().getPhone())
				.build();
	}
}
